...
(() => {
  ...
  // ---------- The AI brain call ----------
  // Works two ways:
  //  1. If a server /api/reply is available (Node hosting), use it.
  //  2. Otherwise (e.g. GitHub Pages, static hosting), use the in-browser brain.
  async function getReply(text) {
    // Try the server brain first, but only briefly.
    try {
      ...
      // No server (static hosting) — fall through to local brain.
    }
    // In-browser brain (public/brain.js exposes window.generateReply).
    if (typeof window.generateReply === "function") {
      try {
        return window.generateReply(text, history.slice(-10));
      } catch (_) {}
    }
